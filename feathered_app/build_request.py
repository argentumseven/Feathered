...
class BuildRequestMixin:
    ...
    def _live_value(self, name: str, default: str = "") -> str:
        control = BuildRequestMixin._live_control(self, name)
        if control is None:
            return default
        try:
            value = control.get()
    ...
    def _selected_arch(self) -> str:
        ...
        """
        snapshot = self.__dict__.get("_arch_snapshot")
        if snapshot is not None:
            return snapshot
        # Called unbound by contract tests whose stub host has no mixin
        # methods, so reach the reader through the class rather than the
        # instance.
        pinned = BuildRequestMixin._build_snapshot_value(self, "target", "arch")
        if pinned is not None:
            return str(pinned)
        variable = self.__dict__.get("arch_var")
        if variable is None:
            # Contract tests drive the cache logic with a stub host that has no
            # widgets at all; an absent control is not an architecture.
            return self.__dict__.get("_last_known_arch", "")
        try:
            return variable.get()
        except RuntimeError:
    ...
    def _selected_output_option(self, field: str, variable: str, *, flag: bool = False):
        ...
        pinned = BuildRequestMixin._build_snapshot_value(self, section, field)
        if pinned is not None:
    ...
    def _selected_content(self, field: str, variable: str) -> str:
        ...
        """
        pinned = BuildRequestMixin._build_snapshot_value(self, "content", field)
        if pinned is not None:
            return str(pinned)
    ...
    def _build_snapshot_value(self, section: str, field: str, default=None):
        ...
        must keep reflecting the live controls. Only the worker is pinned.
        """
        snapshot = self.__dict__.get("_build_snapshot")
        if snapshot is None:
            return None
